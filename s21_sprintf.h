...
void s21_xX_to_str(struct s_struct *struct_frmtt, char *tmp_str, unznaked long long number);
void s21_eE_to_str(struct s_struct *frmt, char *tmp_str, long double number);
void s21_gG_to_str(struct s_struct *frmt, char *tmp_str, long double number);
...
void add_for_fdeEfgG(struct s_struct *frmt, char *tmp_str);  // повторяющийся модуль
