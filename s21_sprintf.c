  ...
  while (*format) {
    ...
    if (*format == '%') {
      ...
      while (s21_check_digital(format) || *format == '*') {
        s21_set_acc(&formats.Wdt, format++, &arg);
        if (formats.Wdt < 0) {
          formats.Wdt *= -1;
          formats.flag_m = 1;
        }
      ...
      if (*format == '.') {
        ...
        formats.accuracy = 0;
        while (s21_check_digital(format) || *format == '*')
          s21_set_acc(&formats.accuracy, format++, &arg);
      }
